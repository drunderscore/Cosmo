...
{
Server::Server(JS::Realm& realm) : Object(*realm.intrinsics().object_prototype()) {}

void Server::initialize(JS::Realm& realm)
{
    Base::initialize(realm);

    define_native_accessor(realm, "map", map_getter, map_setter, 0);

    define_native_function(realm, "createEntityByName", create_entity_by_name, 1, 0);
    define_native_function(realm, "createFakeClient", create_fake_client, 1, 0);
    define_native_function(realm, "getEntityByIndex", get_entity_by_index, 1, 0);
    define_native_function(realm, "emitSound", emit_sound, 2, 0);
    define_native_function(realm, "sayText2", say_text_2, 8, 0);
    define_native_function(realm, "getPlayerByUserId", get_player_by_userid, 1, 0);
    define_native_function(realm, "getEntityByHandle", get_entity_by_handle, 1, 0);
}
...
JS_DEFINE_NATIVE_FUNCTION(Server::map_setter)
{
    ...
    if (!map_name.is_string())
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::NotAString, map_name);
    ...
    if (!Plugin::the().engine_server().IsMapValid(String::formatted("maps/{}.bsp", map_name_string).characters()))
        return vm.throw_completion<JS::Error>("Invalid map");
...
JS_DEFINE_NATIVE_FUNCTION(Server::create_entity_by_name)
{
    ...
    if (!entity_name.is_string())
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::NotAString, entity_name);
...
JS_DEFINE_NATIVE_FUNCTION(Server::create_fake_client)
{
    ...
    if (!client_name.is_string())
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::NotAString, client_name);
...
JS_DEFINE_NATIVE_FUNCTION(Server::get_entity_by_index)
{
    ...
    if (!index.is_number())
        return vm.throw_completion<JS::TypeError>(String::formatted("{} is not a number", index));

    auto* maybe_edict = Plugin::the().engine_server().PEntityOfEntIndex(TRY(index.to_i32(vm)));
    if (!maybe_edict)
        return JS::js_undefined();
    ...
    return Entity::create(*vm.current_realm(), maybe_entity);
}
...
JS_DEFINE_NATIVE_FUNCTION(Server::emit_sound)
{
    ...
    if (entity_or_position.is_object() && is<Entity>(entity_or_position.as_object()))
        ...
        source = TRY(to_source_vector(vm, entity_or_position));

    // Super sanity
    VERIFY(!source.has<Empty>());

    Cosmo::Scripting::emit_sound(vm, vm.argument(0), vm.argument(2), vm.argument(3), vm.argument(4),
...
JS_DEFINE_NATIVE_FUNCTION(Server::say_text_2)
{
    ...
    if (!message.is_string())
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::NotAString, message);
...
JS_DEFINE_NATIVE_FUNCTION(Server::get_player_by_userid)
{
    ...
    if (!user_id.is_number())
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::IsNotA, user_id, "number");
    ...
    for (auto i = 0; i < Plugin::the().engine_server().GetIServer()->GetClientCount(); i++)
    {
        ...
        if (client->GetUserID() == TRY(user_id.to_i32(vm)))
            return Entity::create(*vm.current_realm(), Plugin::the().server_game_ents().EdictToBaseEntity(
...
JS_DEFINE_NATIVE_FUNCTION(Server::get_entity_by_handle)
{
    ...
    if (!handle_number.is_number())
        return vm.throw_completion<JS::TypeError>(JS::ErrorType::IsNotA, handle_number, "number");

    auto handle_number_integer = TRY(handle_number.to_i32(vm));

    if (handle_number_integer == INVALID_NETWORKED_EHANDLE_VALUE)
        return JS::js_undefined();

    CBaseHandle handle(handle_number_integer);
    if (!handle.IsValid())
        return JS::js_undefined();
